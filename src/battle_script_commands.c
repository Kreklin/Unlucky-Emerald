...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        // moves by the player will miss unless there is a 100% chance of hitting
        // moves by the opponent will hit unless there is a 0% chance of hitting
        if ((GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER && calc < 100 && !(  // krek code
            gBattleTypeFlags & (BATTLE_TYPE_WALLY_TUTORIAL))) || calc == 0)         //
        {
            gMoveResultFlags |= MOVE_RESULT_MISSED;
...
static void Cmd_critcalc(void)
{
    ...
    if ((gBattleMons[gBattlerTarget].ability != ABILITY_BATTLE_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_SHELL_ARMOR)
     ...
     && !(GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) // krek code
...
static inline void ApplyRandomDmgMultiplier(void)
{
    ...
    // player's moves are always min rolls (85% damage)
    // opponent's moves are always max rolls (100% damage)
    u16 randPercent;                                            // krek code
...
static void Cmd_adjustnormaldamage(void)
{
    ...
    // focus band always procs for opponent, never for player
    if (holdEffect == HOLD_EFFECT_FOCUS_BAND 
    // && (Random() % 100) < param
     && GetBattlerSide(gBattlerTarget) != B_SIDE_PLAYER   // krek code
    )
    {
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
    {
        ...
        if (statusChanged == TRUE)
        {
            ...
            if (sStatusFlagsForMoveEffects[gBattleCommunication[MOVE_EFFECT_BYTE]] == STATUS1_SLEEP)
                ...
                if (GetBattlerSide(gEffectBattler) == B_SIDE_PLAYER)                              // krek code
                    gBattleMons[gEffectBattler].status1 |= STATUS1_SLEEP_TURN(5);                 //
                else                                                                              //
        ...
        else
        {
            ...
            case MOVE_EFFECT_UPROAR:
                if (!(gBattleMons[gEffectBattler].status2 & STATUS2_UPROAR))
                {
                    ...
                    // player is locked into uproar for 5 turns, opponent is locked in for 2 turns
                    // gBattleMons[gEffectBattler].status2 |= STATUS2_UPROAR_TURN((Random() & 3) + 2); // 2-5 turns
                    if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER)                  // krek code
                        gBattleMons[gEffectBattler].status2 |= STATUS2_UPROAR_TURN(5);      //
                    else                                                                    //
                        gBattleMons[gEffectBattler].status2 |= STATUS2_UPROAR_TURN(2);      //
            ...
            case MOVE_EFFECT_WRAP:
                ...
                {
                    // wrap targetting player lasts 6 turns, wrap targetting opponent lasts 3 turns
                    // gBattleMons[gEffectBattler].status2 |= STATUS2_WRAPPED_TURN((Random() & 3) + 3); // 3-6 turns
                    if (GetBattlerSide(gBattlerTarget) == B_SIDE_PLAYER)                        // krek code
                        gBattleMons[gEffectBattler].status2 |= STATUS2_WRAPPED_TURN(6);         //
                    else                                                                        //
                        gBattleMons[gEffectBattler].status2 |= STATUS2_WRAPPED_TURN(3);        //
            ...
            case MOVE_EFFECT_THRASH:
                ...
                else
                {
                    gBattleMons[gEffectBattler].status2 |= STATUS2_MULTIPLETURNS;
                    gLockedMoves[gEffectBattler] = gCurrentMove;
                    // player thrash for 2 turns, opponent thrash for 3 turns
                    // gBattleMons[gEffectBattler].status2 |= STATUS2_LOCK_CONFUSE_TURN((Random() & 1) + 2); // thrash for 2-3 turns
                    if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER)
                        gBattleMons[gEffectBattler].status2 |= STATUS2_LOCK_CONFUSE_TURN(2);
                    else
...
static void Cmd_seteffectwithchance(void)
{
        ...
        && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        SetMoveEffect(FALSE, MOVE_EFFECT_CERTAIN);
            ...
            // secondary effects never proc for player unless there is a 100% chance
            // secondary effects always proc for opponent unless there is a 0% chance
            // Random() % 100 < percentChance
            ((GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER && percentChance >= 100)     // krek code
            || (GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT && percentChance > 0))   //
...
static void Cmd_setprotectlike(void) // protect and endure
{
    ...
    // protect never succeeds for player, always succeeds for opponent
    // if (sProtectSuccessRates[gDisableStructs[gBattlerAttacker].protectUses] >= Random() && notLastTurn)
    if (GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT)        // krek code
    {
...
static void Cmd_setmultihitcounter(void)
{
    ...
    {
        // multi hit moves always hit 2 times for player and 5 times for opponent
...
static void Cmd_tryKO(void)
{
    ...
    // player focus band never procs, opponent focus band always procs
    if (holdEffect == HOLD_EFFECT_FOCUS_BAND
    //  && (Random() % 100) < param
    ...
    else
    {
        ...
        if (!(gStatuses3[gBattlerTarget] & STATUS3_ALWAYS_HITS))
        {
            ...
            // player only hits if 100% chance of hitting, opponent hits if there is more than a 0% chance of hitting
            // if (Random() % 100 + 1 < chance && gBattleMons[gBattlerAttacker].level >= gBattleMons[gBattlerTarget].level)
            if ((100 < chance && GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER)         // krek code
             || (30 < chance && GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT))      //
                chance = TRUE;
            else
                chance = FALSE;
        }
...
static void Cmd_psywavedamageeffect(void)
{
    ...
    // psywave does max damage against player and min damage against opponent
...
static void Cmd_disablelastusedattack(void)
{
        ...
        && i != MAX_MON_MOVES && gBattleMons[gBattlerTarget].pp[i] != 0)
    {
        ...
        // disable targetting player lasts 5 turns, disable targetting opponent lasts 2 turns
...
static void Cmd_trysetencore(void)
{
        ...
        && i != 4 && gBattleMons[gBattlerTarget].pp[i] != 0)
    {
        ...
        // player get encored for 6 turns, opponent is encored for 3 turns
        // gDisableStructs[gBattlerTarget].encoreTimer = (Random() & 3) + 3;
        if (GetBattlerSide(gBattlerTarget) == B_SIDE_PLAYER)    // krek code
            gDisableStructs[gBattlerTarget].encoreTimer = 6;    //
        else                                                    //
...
static void Cmd_tryspiteppreduce(void)
{
        ...
        && gLastMoves[gBattlerTarget] != 0xFFFF)
    {
        ...
        {
            // spite on player reduces max pp, spite on opponent reduces min pp
            // s32 ppToDeduct = (Random() & 3) + 2;
            s32 ppToDeduct;                                         // krek code
            if (GetBattlerSide(gBattlerTarget) == B_SIDE_PLAYER)    //
                ppToDeduct = 5;                                     //
            else                                                    //
                ppToDeduct = 2;                                     //
...
static void Cmd_presentdamagecalculation(void)
{
    // present deals max damage to player, present heals opponent
...
static void Cmd_magnitudedamagecalculation(void)
{
    // player always magnitude 4, opponent always magnitude 10
    // s32 magnitude = Random() % 100;
    s32 magnitude;                                              // krek code
    if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER)      //
        magnitude = 1;                                          //
    else                                                        //
        magnitude = 99;                                         //
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        {
            u8 shakes;
            odds = 0;           // krek code    Pokeballs only work if odds > 254 or master ball
            shakes = 0;         //
