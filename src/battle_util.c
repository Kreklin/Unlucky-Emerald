        ...
        else
        {
            ...
            case ENDTURN_YAWN:  // yawn
                if (gStatuses3[gActiveBattler] & STATUS3_YAWN)
                {
                    ...
                    {
                        CancelMultiTurnMoves(gActiveBattler);
                        // player sleeps for 5 turns, opponent sleeps for 2
                        if (GetBattlerSide(gActiveBattler) == B_SIDE_PLAYER)
                            gBattleMons[gActiveBattler].status1 |= STATUS1_SLEEP_TURN(5);
                        else
...
u8 AtkCanceller_UnableToUseMove(void)
{
    ...
    do
    {
        ...
        case CANCELLER_FROZEN: // check being frozen
            ...
            {
                // player never unfreezes, opponent always unfreezes
                // if (Random() % 5)
        ...
        case CANCELLER_CONFUSED: // confusion
            if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION)
            {
                ...
                if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION)
                {
                    // player always hits self, opponent never hits self
                    // if (Random() & 1)
        ...
        case CANCELLER_PARALYSED: // paralysis
            // player is always fully paralyzed, opponent is never fully paralyzed
        ...
        case CANCELLER_IN_LOVE: // infatuation
            if (gBattleMons[gBattlerAttacker].status2 & STATUS2_INFATUATION)
            {
                ...
                // player is always immobilized by love
                // opponent is never immobilzed by love
                // if (Random() & 1)
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI)) // Why isn't that check done at the beginning?
    {
        ...
        case ABILITYEFFECT_ON_DAMAGE: // Contact abilities and Color Change
            ...
            case ABILITY_EFFECT_SPORE:
                // player's attacks always proc effect spore, opponent's attacks never proc effect spore
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                 && gBattleMons[gBattlerAttacker].hp != 0
                 && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                 && TARGET_TURN_DAMAGED
            ...
            case ABILITY_POISON_POINT:
                // player always procs poison point, opponent never procs poison point
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                 && gBattleMons[gBattlerAttacker].hp != 0
                 && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                 && TARGET_TURN_DAMAGED
                ...
                break;
            case ABILITY_STATIC:
                // player always procs static, opponent never procs static
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                 && gBattleMons[gBattlerAttacker].hp != 0
                 && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                 && TARGET_TURN_DAMAGED
            ...
            case ABILITY_FLAME_BODY:
                // player always procs flame body, opponent never procs flame body
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                 && gBattleMons[gBattlerAttacker].hp != 0
                 && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                 && (gBattleMoves[move].flags & FLAG_MAKES_CONTACT)
            ...
            case ABILITY_CUTE_CHARM:
                // player always procs cute charm, opponent never procs cute charm
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                 && gBattleMons[gBattlerAttacker].hp != 0
                 && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                 && (gBattleMoves[move].flags & FLAG_MAKES_CONTACT)
    ...
    case ITEMEFFECT_KINGSROCK_SHELLBELL:
        if (gBattleMoveDamage)
        {
            ...
            case HOLD_EFFECT_FLINCH:
                // Attacks on player always proc kings rock, attacks on opponent never proc kings rock
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                    && TARGET_TURN_DAMAGED
